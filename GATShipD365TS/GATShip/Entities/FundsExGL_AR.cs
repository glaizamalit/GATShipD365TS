    ...
    public class FundsExGL_AR
    {
        ...
        public bool Create(int eventId, int a3_id, FundsPayload result, string jsonData, DateTime? received_at, string actionEntity, string category, ref List<string> validationMsg)
        {
            ...
            try
            {
                ...
                string deptForFund = funds.DeptForFund(nominationId, result.data.PIC, result.data.businessType, locCode, entityType, "Fund", result.data.chargeDept, principalNumber, result.data.refNumber, ref validationMsg);
                ...
                for (var i = 1; i <= 2; i++)
                {
                    ...
                    if (action == "CREATE")
                    {
                        detail.LineDescription = (vesselName + " " + typeOfReceipt + " " + voyageCode + " " + nominationFileNumber).Length > 60 ? (vesselName + " " + typeOfReceipt + " " + voyageCode + " " + nominationFileNumber).Substring(0, 60) : (vesselName + " " + typeOfReceipt + " " + voyageCode + " " + nominationFileNumber);
                    }
                    else
                    {
                        detail.LineDescription = (vesselName + " " + typeOfReceipt + " " + voyageCode + " " + nominationFileNumber).Length > 56 ? (vesselName + " " + typeOfReceipt + " " + voyageCode + " " + nominationFileNumber).Substring(0, 56) + "_Rev" : (vesselName + " " + typeOfReceipt + " " + voyageCode + " " + nominationFileNumber) + "_Rev";
                    }
                    ...
                    else //credit
                    {
                        ...
                        detail.Account =  funds.CreditAccountForFCExGL();
                ...
                string folder = Config.ProcessedFolder + Config.A3Folders.FundsExGL_AR.ToString();
        ...
        public bool Modify(int eventId, int a3_id, FundsPayload result, string jsonData, DateTime? received_at, string actionEntity, string category, ref List<string> validationMsg)
        {
            ...
            return Create(eventId, a3_id, result, jsonData, received_at, "FMExGL_AR", category, ref validationMsg);
        }
